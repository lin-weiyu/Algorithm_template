...
void pd(int i,int s,int t){
    int l=(i<<1),r=(i<<1)|1,mid=(s+t)>>1;//l为左子树的节点数，r为右子树所代表的节点数
    if(mul[i]!=1){//先乘再加,因要把加法的tag进行乘法处理
        mul[l]*=mul[i];
        mul[l]%=mod;
        mul[r]*=mul[i];
        mul[r]%=mod;
...
void build(int s,int t,int i){//s:左边界  t:右边界  i:当前节点号
    mul[i]=1;//乘法的lazy_tag初始化应为1
    if(s==t){//为最底层的叶子节点时进行赋予准确值
        sum[i]=a[i];
        return;
    }
    int mid=s+((t-s)>>1);
    build(s,mid,i<<1);//建左子树
    build(mid+1,t,(i<<1)|1);//建右子树
    up(i);//把左右子树的目标值返回到当前节点
}
void chen(int l,int r,int s,int t,int i,ll z){// [l,r]为目标区间
    int mid=s+((t-s)>>1);
    if(l<=s&&t<=r){
        mul[i]*=z;
        mul[i]%=mod;
    ...
    pd(i,s,t);//在改变左右子树前，必须先推tag到左右子树
